...
def pre_screening(list_unit, list_inlet, list_outlet, Action_taken):
    ...
    delta_scoreD = 20   #penalty option 4
    ...
    list_unit_essential = ['mixer_0', 'flash_0', 'reactor_1']
	# list_unit_essential = ['mixer_0', 'flash_0', 'reactor_1', 'heater_1']
    ...
    for k in range(N_list_str):
        ...
        if str_unit_in == 'flash_0' and str_unit_out == 'mixer_0':
            pres_score = pres_score-delta_scoreA

        if str_unit_in == 'exhaust' and str_unit_out == 'mixer_0':
            pres_score = pres_score-delta_scoreA

        # physics constraint 5: exhaust can only connect to splitter or flash
            # when connected to a splitter, it must connect to outlet_2 (less side)
        if str_unit_in == 'exhaust':
            if str_unit_out not in ['flash_1', 'flash_2', 'splitter_1', 'splitter_2']:
                pres_score = pres_score-delta_scoreA
                # print('constraint 5.1 fails')

            elif str_unit_out in ['splitter_1', 'splitter_2'] and str_outlet == 'outlet_2':
                pres_score = pres_score-delta_scoreB
                # print('constraint 5.2 fails')
        ...
        if str_unit_out in ['flash_1', 'flash_2'] and str_outlet == 'liq_outlet': 
            if str_unit_in in ['compressor_1', 'compressor_2', 'turbine_1', 'turbine_2']:
                pres_score = pres_score-delta_scoreA
                
        # physics constraint 7: cannot connect to same kind of units
        if str_unit_out in ['compressor_1', 'compressor_2', 'turbine_1', 'turbine_2']: 
            if str_unit_in in ['compressor_1', 'compressor_2', 'turbine_1', 'turbine_2']:
                pres_score = pres_score-delta_scoreB
                # print('constraint 7.1 fails')
                
        if str_unit_out in ['heater_1', 'heater_2', 'cooler_1', 'cooler_2']: 
            if str_unit_in in ['heater_1', 'heater_2', 'cooler_1', 'cooler_2']: 
                pres_score = pres_score-delta_scoreB
                # print('constraint 7.2 fails')
                
        if str_unit_out in ['reactor_1', 'reactor_2']: 
            if str_unit_in in ['reactor_1', 'reactor_2']:  
                pres_score = pres_score-delta_scoreB
                # print('constraint 7.3 fails')

        # # physics constraint 8.5: heater/cooler cannot be in the upstream of compressor/expander
        # if str_unit_out in ['heater_1', 'heater_2', 'cooler_1', 'cooler_2'] and str_unit_in in ['compressor_1', 'compressor_2', 'turbine_1', 'turbine_2']:
        #     pres_score = pres_score-delta_scoreB
        #     # print('constraint 8.5 fails')
            
    # physics constraint 8: splitter/flash cannot connect to mixer completely
    if 'splitter_1' in list_str_unit_out:
        indices_1 = [i for i,val in enumerate(list_str_unit_out) if val == 'splitter_1']
        if 'mixer_1' in list_str_unit_in:
            indices_3 = [i for i,val in enumerate(list_str_unit_in) if val == 'mixer_1']
            if indices_1 == indices_3:
                pres_score = pres_score-delta_scoreB
        if 'mixer_2' in list_str_unit_in:
            indices_4 = [i for i,val in enumerate(list_str_unit_in) if val == 'mixer_2']
            if indices_1 == indices_4:
                pres_score = pres_score-delta_scoreB
    if 'splitter_2' in list_str_unit_out:
        indices_2 = [i for i,val in enumerate(list_str_unit_out) if val == 'splitter_2']
        if 'mixer_1' in list_str_unit_in:
            indices_3 = [i for i,val in enumerate(list_str_unit_in) if val == 'mixer_1']
    ...
    if 'flash_1' in list_str_unit_out:
        indices_1 = [i for i,val in enumerate(list_str_unit_out) if val == 'flash_1']
        if 'mixer_1' in list_str_unit_in:
            indices_3 = [i for i,val in enumerate(list_str_unit_in) if val == 'mixer_1']
            if indices_1 == indices_3:
                pres_score = pres_score-delta_scoreB
        if 'mixer_2' in list_str_unit_in:
            ...
            if indices_1 == indices_4:
                pres_score = pres_score-delta_scoreB
    if 'flash_2' in list_str_unit_out:
        indices_2 = [i for i,val in enumerate(list_str_unit_out) if val == 'flash_2']
        if 'mixer_1' in list_str_unit_in:
            indices_3 = [i for i,val in enumerate(list_str_unit_in) if val == 'mixer_1']
            if indices_2 == indices_3:
                pres_score = pres_score-delta_scoreB
    ...
    connect2feed = all_units_downstream(list_str_unit_in, list_str_unit_out, 'mixer_0')

    # print('debug-connect2feed: ', connect2feed)
    if 0 in connect2feed:
        # print('constraint 9 fails')
        pres_score = pres_score-delta_scoreA

    # physics constraint 10: it must be in the sequence: feed -> reactor -> flash -> product
    list_str_unit_in_out = list_str_unit_in + list_str_unit_out
    list_End_A = ['mixer_0']
    list_End_B = ['flash_0', 'flash_1', 'flash_2', 'exhaust']

    for End_A in list_End_A:
        for End_B in list_End_B:

            if End_A in list_str_unit_in_out and End_B in list_str_unit_in_out:
                unit_in_route = all_units_between(list_str_unit_in, list_str_unit_out, End_A, End_B)
                if 'reactor_1' not in unit_in_route and 'reactor_2' not in unit_in_route:
                    # print('contraint 10 fails')
                    pres_score = pres_score-delta_scoreA
    ...
    for k in range(len(list_unit)):
        ...
        if str_unit not in ['mixer_0', 'flash_0', 'exhaust']:
            if str_unit == 'mixer_1' or str_unit == 'mixer_2':
                if list_str_unit_in.count(str_unit) < 2 or list_str_unit_out.count(str_unit) < 1:
                    # print('contraint 11 fails: 1')
                    pres_score = pres_score-delta_scoreB
            elif str_unit == 'flash_1' or str_unit == 'flash_2' or str_unit == 'splitter_1' or str_unit == 'splitter_2':
    ...
    # if Action_taken == False and pass_pre_screening == False:
        pres_score = pres_score-delta_scoreB
