...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        for i in range(self.batch_size):
            if not ddone[i]: #Jie...      check done
                q_target[i, eval_act_index[i]] = reward[i] + self.gamma * np.max(q_next[i,:])
            else: #Jie...
                q_target[i, eval_act_index[i]] = reward[i] #Jie...
        ...
        if self.epsilon < 0.4:
            ...
            epsilon_increment = self.epsilon_increment / 20.0
        elif self.epsilon < 0.8:
            epsilon_increment = self.epsilon_increment / 50.0
        else: # 0.8-1.0
            epsilon_increment = self.epsilon_increment / 100.0
